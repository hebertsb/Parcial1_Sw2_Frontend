...
| `seleccion-compra.cjs` | **El agente se entera de lo que el cliente marca tocando** y las acciones nuevas limpian lo correcto. La pantalla le manda por el WebSocket una foto de lo marcado (`contexto`, solo ids): vacía al abrirse la conversación, con la película y la función tras **un clic real en un horario**, y con la **versión** de la última tanda de acciones del servidor ya aplicada; «cambiame el horario» (`compra.quitar_funcion`) deja la película sin función ni butacas, «eliminame la película» (`compra.quitar_pelicula`) la saca y **deja la dulcería**, una compra ya pagada no cuenta y sin cambios no se manda nada; el administrador no manda fotos. Inyecta lo que mandaría el servidor; no usa el modelo ni escribe en la base. |
| `mis-compras.cjs` | **«Mis compras» es solo de quien tiene la sesión**: abre la pantalla real con dos clientes y un administrador y compara lo dibujado contra la base (ninguna compra ajena, ninguna propia que falte; el administrador no ve las de todos), que pida `/ventas/mis-compras`, que la fila se abra con mouse y teclado y que «Boleto» abra su ventana sin abrir la fila. Solo lee (no compra ni cambia nada). Necesita el backend con `GET /ventas/mis-compras` (reconstruir la imagen si es viejo). |
| `pago-stripe.cjs` | **El pago de la compra** (tarjeta con Stripe y efectivo) con el backend y Stripe.js **simulados** (no necesita Docker ni claves): la tarjeta virtual gira al escribir el CVV, RF03 es una casilla real, la venta se crea una sola vez aunque haya reintentos, el navegador verifica con el servidor antes de dar por pagado, y lo que la pantalla le cuenta al agente (solo estados, nunca el número). Uso: `SILENCIO_WAV=... node e2e/pago-stripe.cjs`; con `CAPTURAS=/carpeta` guarda imágenes. No prueba a Stripe de verdad (iframes, 3D Secure): eso se hace con las claves reales. |
...
SILENCIO_WAV=/ruta/silencio.wav node e2e/seleccion-compra.cjs
